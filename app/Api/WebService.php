...
trait WebService
{
    public function cURL($url, $data) {
		...
		curl_setopt($curl, CURLOPT_HTTPHEADER, array('Content-Type: application/json', $this->get_csrf_header())); // Accept JSON response
		...
		echo $output = curl_exec($curl).'output';
		echo $http_code = curl_getinfo($curl, CURLINFO_HTTP_CODE);
		echo $info = curl_getinfo($curl, CURLINFO_HEADER_OUT);  die;
		curl_close($curl);
		
		return [$http_code, json_decode($output)];
    }
